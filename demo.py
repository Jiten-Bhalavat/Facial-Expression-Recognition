...
@cli.command()
@click.argument("image_path", default="Jiten-Bhalavat.png", type=click.Path(exists=True))
@click.option("--mtcnn", is_flag=True, help=mtcnn_help)
def image(image_path, mtcnn):
    """Detect emotions in an image file. IMAGE_PATH is the path to the image (default: Jiten-Bhalavat.png)."""
...
@cli.command()
@click.argument("video_file", default="demo.mp4", type=click.Path(exists=True))
@click.option("--mtcnn", is_flag=True, help=mtcnn_help)
def video(video_file, mtcnn):
    """Analyze emotions in a video file. VIDEO_FILE is the path to the video (default: demo.mp4)."""
